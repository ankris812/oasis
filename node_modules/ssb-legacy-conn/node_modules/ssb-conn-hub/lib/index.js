...
function getKeyFromDhtAddress(addr) {
    var _a = __read(addr.split('~'), 1), transport = _a[0];
    var _b = __read(transport.split(':'), 3), dhtTag = _b[0], remoteId = _b[2];
    if (dhtTag !== 'dht')
        throw new Error('Invalid DHT address ' + addr);
    var key = remoteId[0] === '@' ? remoteId : '@' + remoteId;
    return key;
}
function inferPublicKey(address) {
